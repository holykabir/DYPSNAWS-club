      ...
      <form onSubmit={handleSubmit} style={{ maxWidth: "700px" }}>
        ...
        <div style={{ background: "rgba(107,33,168,0.06)", border: "1px solid rgba(168,85,247,0.12)", borderRadius: "16px", padding: "28px", marginBottom: "20px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            <div><label style={labelStyle}>NAME</label><input style={inputStyle} value={form.name} onChange={(e) => { update("name", e.target.value); update("avatar", e.target.value.split(" ").map(w => w[0]).join("").toUpperCase().slice(0,2)); }} required /></div>
