...
export default function NewEventPage() {
  ...
  return (
    <AdminLayout title="ADD EVENT" subtitle="Create a new event">
      <form onSubmit={handleSubmit} style={{ maxWidth: "800px" }}>
        ...
        >
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            <div style={{ gridColumn: "1 / -1" }}>
              ...
              <input style={inputStyle} value={form.title} onChange={(e) => update("title", e.target.value)} placeholder="Event title" required />
            </div>
            <div>
              <label style={labelStyle}>DATE</label>
              <input style={inputStyle} value={form.date} onChange={(e) => update("date", e.target.value)} placeholder="e.g. April 20, 2026" required />
            </div>
            <div>
              <label style={labelStyle}>LOCATION</label>
              <input style={inputStyle} value={form.location} onChange={(e) => update("location", e.target.value)} placeholder="Venue" required />
            </div>
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  update("color", EVENT_COLORS[EVENT_TYPES.indexOf(e.target.value)] || "#A855F7");
                }}
              >
                {EVENT_TYPES.map((t) => (
        ...
        >
            ...
            <div key={i} style={{ display: "flex", gap: "12px", marginBottom: "10px", alignItems: "center" }}>
              ...
              {form.schedule.length > 1 && (
                <button
                  ...
                  style={{ color: "rgba(245,100,100,0.6)", background: "none", border: "none", cursor: "pointer", fontSize: "18px", flexShrink: 0 }}
                >
                  ×
                </button>
              )}
          ...
          <button
            ...
            style={{ color: "#A855F7", background: "none", border: "none", cursor: "pointer", fontSize: "12px", marginTop: "4px" }}
          >
            + Add time slot
          </button>
        </div>
          ...
          {form.speakers.map((sp, i) => (
            <div key={i} style={{ display: "flex", gap: "12px", marginBottom: "10px", alignItems: "center" }}>
              ...
              {form.speakers.length > 1 && (
                <button type="button" onClick={() => update("speakers", form.speakers.filter((_, j) => j !== i))} style={{ color: "rgba(245,100,100,0.6)", background: "none", border: "none", cursor: "pointer", fontSize: "18px", flexShrink: 0 }}>×</button>
              )}
            </div>
          ))}
          <button type="button" onClick={() => update("speakers", [...form.speakers, { name: "", role: "", topic: "" }])} style={{ color: "#A855F7", background: "none", border: "none", cursor: "pointer", fontSize: "12px", marginTop: "4px" }}>
            + Add speaker
          </button>
        </div>
          ...
          {form.prerequisites.map((p, i) => (
            <div key={i} style={{ display: "flex", gap: "12px", marginBottom: "10px", alignItems: "center" }}>
              <input
                ...
                value={p}
                onChange={(e) => {
                  const arr = [...form.prerequisites];
                  arr[i] = e.target.value;
                  update("prerequisites", arr);
                }}
                placeholder="Prerequisite"
              />
              {form.prerequisites.length > 1 && (
                <button type="button" onClick={() => update("prerequisites", form.prerequisites.filter((_, j) => j !== i))} style={{ color: "rgba(245,100,100,0.6)", background: "none", border: "none", cursor: "pointer", fontSize: "18px", flexShrink: 0 }}>×</button>
              )}
            </div>
          ))}
          <button type="button" onClick={() => update("prerequisites", [...form.prerequisites, ""])} style={{ color: "#A855F7", background: "none", border: "none", cursor: "pointer", fontSize: "12px", marginTop: "4px" }}>
            + Add prerequisite
          </button>
        </div>
